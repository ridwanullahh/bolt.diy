...
### TASK TO PERFORM:
The first goal is to implement a very robust diff edit system for a production safe codebase modification. Currently, i think the AI will rewrite entire file for just some modification. Ensure to implement a very robust dif edit for a production safe codebase modification.

Also, currently, the platform doesnt seems to support AI powered integrated project managment like features such as robust to-do list and AI powered agile software development system. So integrated this for the AI and should always be the first course of action for the AI. The AI should be able to manage the project management. Maybe you can add AI orchestrar project manager for more robust control of AI developer by the AI project manager so they works in harmony. Also while the Project Management System should be AI powered, ensure that human controller can also manage the Project Management system. In other for the project mangement to be persistent, ensure their is like .pm folder for all the project management context. and ensure their is a beautiful interface in the platform for the PM management with both AI and human control.

Also, improve 10x on the AI Long Term memory and codebase indexing and context with additional robust deep research.  You can also add robust .mem folder for more additional context and memory management

Another critical goal for now to make the app use puter js AI API with using puter's claude 4 as core model and puter's gpt-4o, grok-beta, gpt-5, also add ability to add and use custom puter's AI model. So adding puter as provider and puter's models as part of existing models and ensure it's fully functional just like other models and provider. Also you integrate it to every neccessary areas of the application to ensure it fully functional just like other model. And also handle the puter's authentication generously. Important: Don't remove any existing model or provider.
...
below is the full snapshot of their doc; 
...
only using their AI feature. Ensure it's the only AI api the app will be working with for now and ensure, it's fully functional
